...
import { Activity, ArrowRightLeft, BookOpen, Send, ShieldCheck, TrendingUp } from 'lucide-react';
import { useEffect, useMemo, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { MiningDashboard } from './MiningDashboard';
import { PassbookCanvas } from './PassbookCanvas';
...
export function CenterWorkspace({ activeWalletPath }: CenterWorkspaceProps) {
    ...
    const [activeTab, setActiveTab] = useState<'canvas' | 'mining' | 'passbook'>('canvas');
    ...
    const validAmount = /^(?:\d+(?:\.\d{0,8})?|\.\d{1,8})$/.test(normalizedAmount);
    const amountQuanta = useMemo(() => {
        const parsed = Number(normalizedAmount);
        return validAmount && Number.isFinite(parsed) && parsed > 0 ? Math.round(parsed * QUANTA_PER_SCY) : 0;
    ...
    return (
        <section id="workbench-center-workspace" data-tauri-drag-region="false" className="flex min-h-0 flex-1 flex-col overflow-hidden bg-white text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
            <nav className="flex h-10 flex-shrink-0 items-center gap-1 border-b border-zinc-200 bg-zinc-100 px-3 dark:border-zinc-800 dark:bg-zinc-900">
                <button type="button" onClick={() => setActiveTab('canvas')} className={`flex items-center gap-1.5 rounded px-3 py-1.5 text-xs ${activeTab === 'canvas' ? 'bg-white font-semibold text-zinc-900 shadow-sm dark:bg-zinc-950 dark:text-zinc-100' : 'text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100'}`}><Send className="h-3.5 w-3.5" /><ArrowRightLeft className="h-3.5 w-3.5" />{t('workspace.tabCanvas')}</button>
                <button type="button" onClick={() => setActiveTab('mining')} className={`flex items-center gap-1.5 rounded px-3 py-1.5 text-xs ${activeTab === 'mining' ? 'bg-zinc-950 font-semibold text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100'}`}><TrendingUp className="h-3.5 w-3.5" /><Activity className="h-3.5 w-3.5" />{t('workspace.tabMining')}</button>
                <button type="button" onClick={() => setActiveTab('passbook')} className={`flex items-center gap-1.5 rounded px-3 py-1.5 text-xs ${activeTab === 'passbook' ? 'bg-zinc-950 font-semibold text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100'}`}><BookOpen className="h-3.5 w-3.5" /><ShieldCheck className="h-3.5 w-3.5" />{t('workspace.tabPassbook')}</button>
            </nav>
            {activeTab === 'mining' ? <MiningDashboard /> : activeTab === 'passbook' ? <PassbookCanvas activeWalletPath={activeWalletPath} /> : <>
                <div className="flex-1 overflow-y-auto p-4 md:p-6"><div className="mx-auto max-w-4xl space-y-4">
                    ...
                    <div className="grid gap-4 lg:grid-cols-[1fr_0.8fr]">
                        <div className="space-y-3 rounded-xl border border-zinc-200 bg-zinc-50/60 p-4 dark:border-zinc-800 dark:bg-zinc-900/40">
                            ...
                            <label className="block text-xs font-medium">{t('workspace.recipient')}<input value={recipient} onChange={(event) => { setRecipient(event.target.value.trim()); setDraft(null); setStatus(''); }} placeholder="SCY-xxxxxx or scy1..." className={`mt-1 w-full rounded border px-3 py-2 font-mono text-xs dark:bg-zinc-950 ${recipient && !validRecipient ? 'border-rose-500' : 'border-zinc-200 dark:border-zinc-800'}`} /></label>
                            <div className="grid gap-3 sm:grid-cols-2"><label className="block text-xs font-medium">{t('workspace.amount')}<input type="text" inputMode="decimal" value={amount} onChange={(event) => { setAmount(event.target.value.replace(',', '.')); setDraft(null); setStatus(''); }} placeholder="0.50000000" className={`mt-1 w-full rounded border bg-white px-3 py-2 font-mono text-xs dark:bg-zinc-950 ${amount && !validAmount ? 'border-rose-500' : 'border-zinc-200 dark:border-zinc-800'}`} /><span className="mt-1 block font-mono text-[10px] text-zinc-500">= {amountQuanta.toLocaleString()} quanta{amount && !validAmount ? ' · max 8 decimals' : ''}</span></label><label className="block text-xs font-medium">{t('workspace.fee')}<select value={fee} onChange={(event) => { setFee(event.target.value); setDraft(null); setStatus(''); }} className="mt-1 w-full rounded border border-zinc-200 bg-white px-3 py-2 text-xs dark:border-zinc-800 dark:bg-zinc-950"><option value="60">{t('workspace.normalFee')}</option><option value="120">{t('workspace.fastFee')}</option></select></label></div>
                            ...
                            <div className="flex flex-wrap gap-2"><button type="button" disabled={!sender || !validRecipient || !validAmount || amountQuanta === 0} onClick={() => void createDraft()} className="rounded bg-zinc-900 px-3 py-2 text-xs font-medium text-white disabled:opacity-40 dark:bg-zinc-100 dark:text-zinc-950">{t('workspace.draftPayload')}</button><button type="button" disabled={working || !sender || !validRecipient || !validAmount || amountQuanta === 0} onClick={() => setShowPin(true)} className="flex items-center gap-1.5 rounded bg-blue-600 px-3 py-2 text-xs font-medium text-white disabled:opacity-40"><Send className="h-3.5 w-3.5" />{t('workspace.signBroadcast')}</button></div>

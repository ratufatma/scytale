...
#[test]
fn test_indexer_auto_catchup_on_startup() {
    let dir = tempfile::TempDir::new().unwrap();
    let db_path = dir.path().join("db");
    let payout = vec![0x12, 0x34, 0x56];

    // 1. Start node and mine at least 2 blocks so redb storage has height >= 2
    {
        let mut cfg = test_config(db_path.clone(), true);
        cfg.miner_payout_script = payout.clone();
        let mut node = Node::open(cfg).unwrap();
        node.start().unwrap();
        assert!(wait_for_height(&node, 2, 5000), "must mine to height 2");
        node.shutdown().unwrap();
    }

    // 2. Open node again without mining. Remove SQLite indexer file to simulate lag/wipe
    let sqlite_path = db_path.join("indexer.sqlite");
    if sqlite_path.exists() {
        let _ = std::fs::remove_file(&sqlite_path);
    }

    // 3. Re-open node. Node::open should auto-detect that indexer is behind redb and execute auto-catchup!
    let mut node2 = Node::open(test_config(db_path, false)).unwrap();
    node2.start().unwrap();

    let indexer_arc = node2.indexer_store().expect("indexer store must be active");
    let indexer = indexer_arc.lock().unwrap();

    let canonical_height = node2.canonical_height();
    assert!(canonical_height >= 2);

    let indexed_height = indexer.get_latest_indexed_height().unwrap();
    assert_eq!(
        indexed_height,
        Some(canonical_height),
        "SQLite indexer must be backfilled to match canonical height"
    );
    assert!(!indexer.requires_catchup(canonical_height).unwrap());

    // 4. Verify that the mined coinbase address has accumulated balance in SQLite indexer
    let b1 = node2.storage_handle().get_block_by_height(1).unwrap().unwrap();
    let p = scytale_indexer::extract_block_indexer_payload(&b1);
    let recipient_addr = p[0].outputs[0].address.clone();
    let balance = indexer.get_address_balance(&recipient_addr).unwrap();
    assert!(balance >= 2_500_000_000, "Coinbase recipient should have indexed balance");
